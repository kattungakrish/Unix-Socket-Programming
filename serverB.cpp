...
int main(){
  //BACKEND SERVER B BOOTUP

  //Code Segment to read and store data from the file input.
  /*NOTE: This segment take input from the specific file dataB.txt.
  Incase of using a different file name change the name accordingly
  at the field indicated below.*/
  string line;
  ifstream datafile_test_cases("dataB.txt"); //**Kindly change the filename here if taking input from a different file name.
  int count = 0;
  int duplicate_count;
  ...
  map<string, map<string, vector<string> > > data_set_b;
  /* Data set of B (2D Map): Stores country name as the keys and
...
//Reading the file vector to store and build the backend server dataset : data_set_b (Data set of B)
  for(i=0; i<file_input_vector.size(); i++){
    if(int(file_input_vector[i][0])>=48 && int(file_input_vector[i][0])<=57){//Main if
      ...
      if(country_name!=" "){
        ...
        element of the 2D map data_set_b.*/
      data_set_b.insert(pair<string,map<string, vector<string> > >(country_name, group));
...
//BACKEND SERVER B UDP SOCKET SETUP
...
struct addrinfo *server_b_info; //pointer to a linked list of struct addrinfos which are the results of getaddrinfo().
...
if((status = getaddrinfo(NULL, MYPORT, &hints, &server_b_info))!=0){
  std::cerr << "getaddrinfo error: "<< gai_strerror(status) << '\n';
  return 1; //exit main if error.
}

/*NOTE:server_b_info is initialized to the first addrinfo struct of the linked
list of addrinfos it points to. For my program we only need the first one to create a
socket fd and bind it to the server B PORT (31362)*/

//Getting the file descriptor- socket()
socket_fd = socket(server_b_info->ai_family, server_b_info->ai_socktype, server_b_info->ai_protocol);
if(socket_fd == -1){
  ...
  if(bind(socket_fd, server_b_info->ai_addr, server_b_info-> ai_addrlen)== -1){
    close(socket_fd);
    perror("server:bind");
    exit(1);
  }
//If server_b_info struct is NULL, then no server address to bind to, hence server fails to bind.
if(server_b_info==NULL){
  std::cerr << "server: failed to bind."<< '\n';
  exit(1);
}
//SERVER B BOOTUP COMPLETE.
cout<<'\n'<<"Server B is up and running using UDP on port "<<MYPORT<<"."<<'\n';
//cout<<"Done with binding"<<'\n';
...
//Sending the no. of countries handled by this backend server (B).
...
//Sending all the country names handled by this backend server (B) one by one.
...
cout<<"Server B has sent a country list to Main Server."<<'\n';
...
while(1){
  ...
  cout<<"Server B has received a request for finding possible friends of User "<< server_request_userid<<" in "<<server_request_country<<"."<<'\n';
  ...
  for(itr_map2= data_set_b[server_request_country].begin(); itr_map2!= data_set_b[server_request_country].end(); itr_map2++){
    match =0;
...
string server_b_response;
  if(match_count!=0){
    ...
    cout<<'\n'<<"Server B found the following possible friends for User "<<server_request_userid<<" in "<<server_request_country<<": ";
    ...
    for(itr = possible_friends.begin(); itr!= possible_friends.end(); itr++){
      server_b_response = *itr;
      cout<< *itr<<",";
      serv_b_response_len = server_b_response.size();
      char serv_b_response[serv_b_response_len];
      for(k=0; k<serv_b_response_len; k++){
        serv_b_response[k] = server_b_response[k];
      }

      bytes_sent = sendto(socket_fd,serv_b_response, serv_b_response_len, 0, (struct sockaddr*)&server_main_addr, server_main_addr_len);
      if(bytes_sent==-1){
        std::cerr << "Couldn't send." << '\n';
      }
    }
    cout<<"\n";
    cout<<'\n'<<"Server B has sent the result(s) to Main Server."<<'\n';
  }
    ...
    cout<<"Server B has sent 'User "<<server_request_userid<<" not found' to Main Server."<<'\n';
